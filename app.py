...
if generate_btn and product:
    with st.spinner("AI 正在調動五層記憶金字塔進行深度思考..."):
        ...
        with col1:
            st.info(f"### 1. 使用情境\n{parts[1] if len(parts)>1 else '生成中...'}")
            st.info(f"### 2. 根源痛點\n{parts[2] if len(parts)>2 else '生成中...'}")
            st.info(f"### 3. 使用頻率\n{parts[3] if len(parts)>3 else '生成中...'}")
        with col2:
            st.success(f"### 4. 回訪觸發\n{parts[4] if len(parts)>4 else '生成中...'}")
            st.success(f"### 5. 收費觸發\n{parts[5] if len(parts)>5 else '生成中...'}")
            st.success(f"### 6. 牽引能力\n{parts[6] if len(parts)>6 else '生成中...'}")
        with col3:
            st.warning(f"### 7. 門檻行動\n{parts[7] if len(parts)>7 else '生成中...'}")
            st.warning(f"### 8. 依賴風險\n{parts[8] if len(parts)>8 else '生成中...'}")
            st.warning(f"### 9. 停止訊號\n{parts[9] if len(parts)>9 else '生成中...'}")
        
        st.markdown("---")
        with st.expander("查看 AI 原始深度報告"):
            st.write(res)
